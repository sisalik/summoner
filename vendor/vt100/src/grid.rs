...
pub struct Grid {
    ...
    // total rows ever pushed into scrollback; never decremented, even when
    // old rows are evicted from the ring
    scrolled_lines: u64,
}
...
impl Grid {
    ...
    /// Total rows ever scrolled off the top of the screen into scrollback.
    ///
    /// Combined with the scrollback offset this gives every row a stable
    /// "stream row" identity: the row currently drawn at visible row `v` is
    /// stream row `scrolled_lines() - scrollback() + v`, and that number does
    /// not change as further output scrolls the content upward.
    ...
    /// offset.
    ///
    /// Returns `None` if the row has already been evicted from the scrollback
    /// ring, or lies below the bottom of the live screen.
    pub fn stream_row(&self, stream_row: u64) -> Option<&crate::row::Row> {
        if stream_row >= self.scrolled_lines {
            let idx = stream_row - self.scrolled_lines;
            usize::try_from(idx).ok().and_then(|i| self.rows.get(i))
        } else {
            let oldest =
                self.scrolled_lines - self.scrollback.len() as u64;
            if stream_row < oldest {
                return None;
            }
            usize::try_from(stream_row - oldest)
                .ok()
