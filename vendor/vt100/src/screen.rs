...
impl Screen {
    ...
    /// Returns the total number of rows that have ever scrolled off the top
    /// of the screen into scrollback.
    ///
    /// The row drawn at visible row `v` is stream row
    /// `scrolled_lines() - scrollback() + v`, an identity that stays stable
    /// as further output pushes the content upward. Use it with
    /// [`Screen::stream_cell`], [`Screen::stream_row_contents`] and
    /// [`Screen::stream_row_wrapped`] to read a row without disturbing the
    ...
    /// Returns whether the row at `stream_row` wraps into the next row.
    ///
    /// Returns `None` if that row is no longer retained.
    #[must_use]
    pub fn stream_row_wrapped(&self, stream_row: u64) -> Option<bool> {
        self.grid()
            .stream_row(stream_row)
            .map(crate::row::Row::wrapped)
    }

    /// Returns the text contents of `stream_row`, restricted to the given
    /// subset of columns.
    ///
    /// Newlines are not included. Returns `None` if that row is no longer
    /// retained.
    #[must_use]
    pub fn stream_row_contents(
        &self,
        stream_row: u64,
        start: u16,
        width: u16,
    ) -> Option<String> {
        self.grid().stream_row(stream_row).map(|row| {
    ...
    /// Returns the cells of `stream_row`, left to right.
    ///
    /// Cheaper than calling [`Screen::stream_cell`] per column, which repeats
    /// the row lookup every time. Returns `None` if that row is no longer
    /// retained.
    #[must_use]
    pub fn stream_row_cells(
        &self,
        stream_row: u64,
    ) -> Option<impl Iterator<Item = &crate::Cell>> {
        self.grid()
            .stream_row(stream_row)
            .map(crate::row::Row::cells)
    }

    /// Returns the cell at `col` of `stream_row`.
    ///
    /// Returns `None` if that row is no longer retained, or the column is out
    /// of range.
    #[must_use]
    pub fn stream_cell(
        &self,
